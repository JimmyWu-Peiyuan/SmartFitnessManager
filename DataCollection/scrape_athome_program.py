# Code block to scrape, clean and store at home programs
...
def scrape_athome_programs():
    # Set up the Selenium WebDriver
    driver = webdriver.Chrome()  # Make sure ChromeDriver is installed and in your PATH

    # List of URLs to scrape for at home programs
    urls = ['https://www.muscleandstrength.com/workouts/home']
    ...
    for url in urls:
        ...
        for link in all_links:
            href = link.get('href')
            if href and '/workouts/' in href and '/home' not in href:  # Filter out '/home' and repetitions
                workout_links.add(href)

    # Extract only the part after 'workouts/' and eliminate those ending with '.html', 'men', 'women', and 'muscle-building?'
    workout_subpaths = [link.split('/workouts/')[-1] for link in workout_links 
                        if not link.endswith('.html') 
                        and link.split('/workouts/')[-1] not in ['men', 'women']
...
# df.to_csv('Muscle_and_Strength_Workout_Programs_1.xlsx', index=False)
