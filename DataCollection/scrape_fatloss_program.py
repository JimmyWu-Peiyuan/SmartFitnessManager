# Code block to scrape, clean and store fat-loss programs
...
def scrape_fat_loss_programs():
    # Set up the Selenium WebDriver
    driver = webdriver.Chrome()  # Make sure ChromeDriver is installed and in your PATH

    # List of URLs to scrape for fat-loss programs
    urls = [
        'https://www.muscleandstrength.com/workouts/fat-loss',
        'https://www.muscleandstrength.com/workouts/fat-loss?page=1',
        'https://www.muscleandstrength.com/workouts/fat-loss?page=2'
    ]
    ...
    for url in urls:
        ...
        for link in all_links:
            href = link.get('href')
            if href and '/workouts/' in href and '/fat-loss' not in href:  # Filter out '/fat-loss' and repetitions
    ...
    workout_subpaths = [link.split('/workouts/')[-1] for link in workout_links 
                        if not link.endswith('.html') and link.split('/workouts/')[-1] not in ['men', 'women','home']]
...
# df.to_excel('fat_loss_workout_details_with_srno.xlsx', index=False)
